...
But 29 of its 82 runs never happened. The response came back with Anthropic's `stop_reason: "refusal"` (the AI SDK surfaces it as `content-filter`), usually on the very first step, before a single tool call, in response to prompts like a JSON Patch library whose rollback leaves the document half-modified. Nine output tokens, no message, no work.

It was not throttling. Refusals arrive as HTTP 200 with usage reported, and the `anthropic-ratelimit-*` headers on a refused request showed 3,999 of 4,000 requests and 3.999 million of 4 million input tokens still available in the window. It was a safety decision, and the API says so precisely: Fable 5 runs safety classifiers on requests, and refused responses carry a `stop_details` field naming the one that fired. Every refusal we inspected reports `category: "cyber"`, with an explanation that the request "triggered restrictions on violative cyber content" under Anthropic's Usage Policy. A bug report about a JSON-patch rollback, aimed at a workspace with read, write, and test tools, reads as offensive cyber activity to the classifier guarding Anthropic's most safety-hardened model. (Per Anthropic's docs, input tokens are not billed when the refusal fires before output.)

Knowing which classifier fired does not make the behavior predictable. Within one six-minute window, 54 percent of runs refused and specific tasks refused three out of three; the same account then ran 18 consecutive runs with zero refusals in the following three minutes; an exact replica of one of those successful configurations refused three out of three when retried ninety minutes later. Swapping instruction wording, removing test files, and removing the test-running tool all failed to flip a refusing window, while a near-identical request with a shorter system prompt and one fewer tool sailed through in the same minute a byte-identical replay was being refused. Wherever the classifier's decision boundary sits, ordinary debugging work is close enough to it that tiny, semantically empty differences in the request, and nothing visible at all over time, move requests across it. The full probe matrix is a script in the repo.

Fable 5 is explicitly documented as carrying extra safety measures, so some refusal surface is the intended product, and Anthropic ships a remedy: an opt-in `fallbacks` parameter (beta) that reruns a refused request on another model automatically, plus SDK middleware where that is unavailable. That restores availability, but notice what it means for a workload like this one: with fallback configured, your "Fable 5 agent" is quietly a different model for 40 percent of its requests. Either way the operational lesson stands. On this workload, on this day, Fable 5 declined 40 percent of ordinary bug-fix requests, at rates that swung between zero and one hundred percent within the hour. If it is in your stack, your traces need to be watching stop reasons and `stop_details`, because your users will find this behavior before your eval suite does.
